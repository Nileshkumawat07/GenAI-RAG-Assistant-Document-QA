...
class RAGService:
    def __init__(self):
        ...
        self.indexes_by_session: Dict[str, faiss.IndexFlatIP] = {}
        self.chunk_map_by_session: Dict[str, List[DocumentChunk]] = {}
    ...
    async def ingest(self, upload: UploadFile, session_id: str):
        ...
        self._build_session_index(session_id, chunks)
    ...
    def _build_session_index(self, session_id: str, chunks: List[DocumentChunk]) -> None:
        self.chunk_map_by_session[session_id] = chunks
        if not chunks:
            self.indexes_by_session.pop(session_id, None)
            return
    ...
    def _retrieve(self, question: str, session_id: str, chunks: List[DocumentChunk]) -> List[DocumentChunk]:
        ...
        if session_index is not None:
            self._load_model()
            query_embedding = self.embedding_model.encode([expanded_question], convert_to_numpy=True)
            norms = np.linalg.norm(query_embedding, axis=1, keepdims=True)
            query_embedding = query_embedding / np.clip(norms, 1e-12, None)
        ...
        for index, chunk in enumerate(chunks):
            ...
            total_score = lexical_score + (semantic_score * 1.35)
    ...
    def _get_or_load_session_document(self, session_id: str) -> Optional[SessionDocument]:
        ...
        if existing:
            if session_id not in self.indexes_by_session:
                self._build_session_index(session_id, existing.chunks)
            return existing
    ...
    def _generate_answer(self, question: str, chunks: List[DocumentChunk]) -> str:
        ...
        fallback_answer = self._extract_answer_from_context(question, chunks)
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    },
                    {
                        ...
                        "content": (
                            f"Question: {question}\n\n"
                            f"Preferred answer style: {response_style}\n\n"
                            f"Document context:\n{context}\n\n"
    ...
    def _extract_query_phrases(self, question: str) -> List[str]:
        ...
        if len(words) >= 2:
            phrases.extend(" ".join(words[index:index + 2]) for index in range(len(words) - 1))
    ...
    def _expand_with_neighbors(self, indexes: List[int], chunk_count: int) -> List[int]:
        ...
        limit = min(chunk_count, max(TOP_K_RESULTS + 2, len(indexes) * 2))
    ...
    def _score_term_density(self, question_terms: List[str], chunk_terms: List[str]) -> float:
        ...
        question_set = set(question_terms)
        matches = sum(1 for token in chunk_terms if token in question_set)
    ...
    def _extract_answer_from_context(self, question: str, chunks: List[DocumentChunk]) -> str | None:
        question_terms = self._tokenize(self._expand_question(question))
        question_phrases = self._extract_query_phrases(question)
        candidates = []

        for chunk in chunks:
            for line in [line.strip() for line in chunk.text.splitlines() if line.strip()]:
                line_terms = self._tokenize(line)
                score = self._score_chunk(question, question_terms, question_phrases, line, line_terms)
                if re.search(r"\b[\w.+-]+@[\w-]+\.[\w.-]+\b", line):
                    score += 0.25
                if re.search(r"\b(?:\+?\d[\d\s().-]{8,}\d)", line):
                    score += 0.2
                if score > 0:
        ...
        best_lines = [item[2] for item in candidates[:3]]
        best_lines = list(dict.fromkeys(best_lines))

        if any(keyword in question.lower() for keyword in ["skills", "projects", "education", "experience", "certifications"]):
            lines = [f"{index}. {line}" for index, line in enumerate(best_lines, start=1)]
    ...
    def _response_style(self, question: str) -> str:
        lowered = question.lower()
        if any(word in lowered for word in ["how", "steps", "procedure", "process", "instructions"]):
            return "Use numbered steps if the document provides a process."
        if any(word in lowered for word in ["summary", "overview", "summarize"]):
            return "Start with a short summary and then list the important details."
    ...
    def _answer_direct_field(self, question: str, text: str) -> str | None:
        ...
        field_extractors = [
            ({"email", "mail"}, "Email Address", self._extract_email(text)),
            ({"phone", "mobile", "contact", "number"}, "Contact Number", self._extract_phone(text)),
            ({"linkedin"}, "LinkedIn Profile", self._extract_link(text, "linkedin")),
            ({"github"}, "GitHub Profile", self._extract_link(text, "github")),
            ({"portfolio", "website", "site"}, "Website", self._extract_link(text, None)),
            ({"name"}, "Candidate Name", self._extract_name(text)),
            ({"address"}, "Address", self._extract_address(text)),
            ({"location", "city"}, "Location", self._extract_location(text)),
        ]
        ...
        for variant in variants:
            ...
            for token in name_tokens:
                if token in variant_lower:
                    score += len(token)
            if name_tokens and variant_lower.startswith(tuple(name_tokens)):
                score += 2
    ...
    def _name_tokens(self, name: str | None) -> List[str]:
        if not name:
            return []
        return [token.lower() for token in re.findall(r"[A-Za-z]+", name) if len(token) >= 4]
    ...
    def _reset_session_state(self, session_id: str) -> None:
        self.documents_by_session.pop(session_id, None)
        self.indexes_by_session.pop(session_id, None)
        self.chunk_map_by_session.pop(session_id, None)
